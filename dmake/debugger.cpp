...
void Debugger::on_variable_access(const std::string& var_name,
    ...
    for (const auto& bp : breakpoints_) {
        if (bp.enabled && bp.type == Breakpoint::Type::VARIABLE && bp.variable == var_name) {
            std::cerr << "(dmake) Variable breakpoint " << bp.id << " hit: "
...
void Debugger::interactive_loop(const std::string& file, size_t row,
    ...
    std::cerr << "(dmake) " << file << ":" << row << "  " << identifier << "(";
    for (size_t i = 0; i < raw_args.size(); ++i) {
        if (i > 0) std::cerr << " ";
        std::cerr << serialize_argument(raw_args[i]);
    }
    ...
    while (true) {
        int key_type = 0;
        auto result = input_fn_("(dmake) > ", key_type);
        if (!result) {
            if (key_type == 1) {
...
bool Debugger::execute_debugger_command(const std::string& input) {
    ...
    if (cmd == "up") {
        ...
        int cur = (selected_frame_ < 0) ? 0 : selected_frame_;
        if (cur + 1 > max_frame) {
    ...
    if (cmd == "down") {
        if (selected_frame_ <= 0) {
            ...
            select_frame(selected_frame_ - 1);
        }
        return false;
    }
...
std::pair<std::string, size_t> Debugger::selected_file_row() const {
    ...
    auto& stack = interp_.get_trace_stack();
    // Frame 0 = top of stack (most recent call), which is stack.back()
...
void Debugger::show_backtrace() {
    ...
    // Frame #0 is always the current position
    char marker = (selected_frame_ < 0) ? '>' : ' ';
    std::cerr << marker << " #0  " << current_file_ << ":" << current_row_ << "  (current)\n";
