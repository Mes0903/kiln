...
std::string ExecutableArtifact::get_output_path(const std::string& build_dir, const std::string& script_dir) const {
    return (std::filesystem::path(script_dir) / build_dir / get_output_name()).string();
}

std::string LibraryArtifact::get_output_path(const std::string& build_dir, const std::string& script_dir) const {
    bool is_shared = (type_ == ArtifactType::SHARED_LIBRARY);
    std::string lib_name = "lib" + get_output_name() + (is_shared ? ".so" : ".a");
    return (std::filesystem::path(script_dir) / build_dir / lib_name).string();
}
...
void ExecutableArtifact::generate_tasks(BuildGraph& graph, const std::string& build_dir, const std::string& script_dir) {
    ...
    // 2. Create Link Task
    std::string output_path = get_output_path(build_dir, script_dir);
    BuildTask link;
    link.id = output_path;
