...
void register_target_builtins(Interpreter& interp) {
    ...
    auto add_sources_to_target = [](Interpreter& interp, const std::shared_ptr<Target>& target, const std::string& src_dir, const std::vector<std::string>& sources) -> bool {
        ...
        for(const auto& file : sources) {
            ...
            if (p.is_absolute()) {
                normalized_src = p.lexically_normal().string();
                normalized_bin = normalized_src;
            } else {
                // Relative paths could be relative to source or binary dir
                normalized_src = (std::filesystem::path(src_dir) / p).lexically_normal().string();
                normalized_bin = (std::filesystem::path(bin_dir) / p).lexically_normal().string();
                p = std::filesystem::path(src_dir) / p;
            }

            if (!std::filesystem::exists(p)) {
                // Check if this is a generated file (output of add_custom_command)
                // Custom commands store outputs relative to binary dir
                if (custom_rules.find(normalized_src) != custom_rules.end() ||
                    custom_rules.find(normalized_bin) != custom_rules.end()) {
                    continue; // File is generated by custom command - skip existence check
                }
                // Check if GENERATED property is set (check both paths)
                auto check_generated = [&](const std::string& path) -> bool {
                    auto sp_it = source_props.find(path);
                    if (sp_it != source_props.end()) {
                        auto gen_it = sp_it->second.find("GENERATED");
                        if (gen_it != sp_it->second.end() && !interp.is_falsy(gen_it->second)) {
                            return true;
                        }
                    }
                    return false;
                };
                if (check_generated(normalized_src) || check_generated(normalized_bin)) {
                    continue; // File is marked as GENERATED - skip existence check
                }
    ...
    interp.add_builtin("target_sources", [get_target_from_name](Interpreter& interp, const std::vector<std::string>& args) {
        ...
        // Helper to validate sources exist (skips generated files from add_custom_command, GENERATED property, and genex)
        const auto& custom_rules = interp.get_custom_command_rules();
        const auto& source_props = interp.get_source_properties();
