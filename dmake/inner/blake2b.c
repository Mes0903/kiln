 ...
 */
static void dmake_blake2b_increment_counter(blake2b_state* state,
                                              const uint64_t inc)
{
    state->t[0] += inc;
 ...
 */
void dmake_blake2b_update(blake2b_state* state,
                            ...
                            size_t inlen)
{
    ...
    if (inlen > fill)
    {
        ...
        dmake_blake2b_increment_counter(state, BLAKE2B_BLOCKBYTES);
        F(state, state->buf);
        in += fill;
        inlen -= fill;
 ...
 */
void dmake_blake2b_init(blake2b_state* state,
                          size_t outlen,
                          const void* key,
                          size_t keylen)
{
    ...
    if (keylen > 0)
    {
        ...
        dmake_blake2b_update(state, block, BLAKE2B_BLOCKBYTES);
...
void dmake_blake2b_final(blake2b_state* state, void* out, size_t outlen)
{
    (void)(outlen);
    uint8_t buffer[BLAKE2B_OUTBYTES] = {0};
    size_t i;

    dmake_blake2b_increment_counter(state, state->buflen);
                     ...
                     size_t keylen)
{
    ...
    dmake_blake2b_init(&state, outlen, key, keylen);
    dmake_blake2b_update(&state, (const uint8_t*)input, inlen);
    dmake_blake2b_final(&state, output, outlen);
}
