...
static void kiln_sha1_transform(uint32_t state[5], const unsigned char buffer[64])
{
    uint32_t a, b, c, d, e;
    typedef union
    {
...
/* kiln_sha1_init - Initialize new context */
...
void kiln_sha1_update(SHA1_CTX* context,
                      ...
                      size_t len)
{
    ...
    if ((j + len) > 63)
    {
        ...
        kiln_sha1_transform(context->state, context->buffer);
        for (; i + 63 < len; i += 64)
        {
            kiln_sha1_transform(context->state, &data[i]);
        }
        j = 0;
    }
    else
...
void kiln_sha1_final(unsigned char digest[20], SHA1_CTX* context)
{
    ...
    c = 0200;
    kiln_sha1_update(context, &c, 1);
    while ((context->count[0] & 504) != 448)
    {
        c = 0000;
        kiln_sha1_update(context, &c, 1);
    }
    kiln_sha1_update(context, finalcount, 8);
    for (i = 0; i < 20; i++)
    {
        digest[i] =
