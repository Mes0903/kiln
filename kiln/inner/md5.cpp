...
void kiln_md5_transform(MD5_CTX *ctx, const uint8_t data[])
{
...
void kiln_md5_init(MD5_CTX *ctx)
{
    ctx->datalen = 0;
    ctx->bitlen = 0;
...
void kiln_md5_update(MD5_CTX *ctx, const uint8_t data[], size_t len)
{
    size_t i;
    ...
    for (i = 0; i < len; ++i)
    {
        ...
        if (ctx->datalen == 64)
        {
            kiln_md5_transform(ctx, ctx->data);
            ctx->bitlen += 512;
            ctx->datalen = 0;
        }
    }
...
void kiln_md5_final(MD5_CTX *ctx, uint8_t hash[])
{
    size_t i;
